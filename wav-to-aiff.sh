...
# Converts WAV files to uncompressed AIFF format
# Preserves metadata and copies cover art
#
# Usage: wav-to-aiff.sh [OPTIONS] <source_folder> <destination_folder>
#
# Options:
...
usage() {
    ...
    echo ""
    echo "Converts all WAV files in source_folder to uncompressed AIFF format."
    ...
    echo "  $0 \"./Album [WAV]\" \"./Album [AIFF]\""
    echo ""
    echo "  # Convert to Red Book CD quality"
    echo "  $0 --redbook \"./Album [WAV]\" \"./Album [AIFF]\""
    echo ""
    echo "  # Downsample to 48kHz / 24-bit"
    echo "  $0 -r 48000 -b 24 \"./Album [WAV]\" \"./Album [AIFF]\""
    exit 0
}
...
echo -e "${GREEN}WAV to AIFF Converter${NC}"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
# Count WAV files
WAV_COUNT=$(find "$SOURCE_DIR" -maxdepth 1 -iname "*.wav" -type f | wc -l | tr -d ' ')

if [ "$WAV_COUNT" -eq 0 ]; then
    echo -e "${YELLOW}Warning: No WAV files found in source folder${NC}"
    exit 0
fi

echo -e "Found ${GREEN}$WAV_COUNT${NC} WAV files to convert"
echo ""

# Convert each WAV file
CURRENT=0
for wav_file in "$SOURCE_DIR"/*.wav "$SOURCE_DIR"/*.WAV; do
    [ -e "$wav_file" ] || continue
    ...
    echo -e "[${CURRENT}/${WAV_COUNT}] Converting: ${YELLOW}$filename_noext${NC}"

    if ffmpeg -y -i "$wav_file" "${FFMPEG_OPTS[@]}" "$output_file" 2>/dev/null; then
